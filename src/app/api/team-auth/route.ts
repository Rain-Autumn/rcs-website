...
export async function POST(request: NextRequest) {
  ...
  const passwordHash = process.env.RCS_TEAM_PASSWORD_HASH || '';
  const reservation = reservePasswordAttempt(request, 'team', passwordHash);
  ...
  if (!body || typeof body.password !== 'string' || body.password.length > 200 || !(await verifyTeamPassword(body.password))) {
