...
export default async function ResearchPage({ params }: PageProps) {
  ...
  const adminCopy = {
    fr: [
      ...
      "Accès réservé. Saisissez le mot de passe pour ouvrir le formulaire de publication.",
    ],
    en: [
      "03 // PUBLICATION",
      "Publish research",
      "Restricted access. Enter the password to open the publication form.",
    ],
    nl: [
      "03 // PUBLICATIE",
      "Onderzoek publiceren",
      "Beperkte toegang. Voer het wachtwoord in om het publicatieformulier te openen.",
    ],
  }[locale];
      ...
      <main id="main" className="research-page">
        ...
        <section className="technical-panel" data-section="01">
          <div className="section-heading">
            ...
            <p className="section-lead">{copy.catalogLead}</p>
          </div>
          <div className="research-project-grid">
            ...
            {publications.map((project) => (
              <article
                className="research-project-card research-project-card--published"
                key={project.id}
              >
                <div className="research-project-meta">
                  <strong>{project.id}</strong>
                ...
                <div className="technology-strip">
                  {project.evidence.map((kind) => (
                    <span key={kind}>{kind.toUpperCase()}</span>
                  ))}
                </div>
                <a
                  className="mechanical-button"
                  href={`/api/research-files/${project.id}`}
                >
        ...
        <section className="technical-panel" data-section="DATA">
          <div className="section-heading">
            <p className="eyebrow">RCS // EVIDENCE</p>
            <h2>{copy.evidenceTitle}</h2>
          </div>
          <div className="evidence-grid">
            {Object.entries(copy.evidence).map(([kind, item]) => (
              <article key={kind} data-kind={kind}>
                <span>{item[0]}</span>
                <p>{item[1]}</p>
              </article>
            ))}
        ...
        <section className="technical-panel research-support" data-section="02">
          <div className="section-heading">
            <p className="eyebrow">{adminCopy[0]}</p>
            <h2>{adminCopy[1]}</h2>
            <p className="section-lead">{adminCopy[2]}</p>
          </div>
