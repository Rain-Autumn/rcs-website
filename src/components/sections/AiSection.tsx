...
export function AiSection({ copy }: { copy: RcsCopy["intelligence"] }) {
  ...
  const activeAgent =
    copy.agents.find((item) => item.id === active) ?? copy.agents[0];

  const registerNode = useCallback(
    (id: AgentId, node: HTMLButtonElement | null) => {
  ...
  const calculateRoutes = useCallback(() => {
    ...
    const children = copy.agents
      ...
      .map((item) => {
        ...
        return element
          ? { id: item.id, rect: element.getBoundingClientRect() }
          : null;
      })
    ...
    if (mobile) {
      ...
      const lastY = Math.max(
        ...destinations.map((destination) => destination.y),
      );
    ...
    const junctionY =
      available > 80 ? start.y + available * 0.42 : start.y + 42;
  ...
  return (
    <section
      className="technical-panel ai-section"
      id="intelligence"
      data-section="05"
    >
      <SectionHeading
      ...
      <div className="agent-map" data-reveal ref={mapRef}>
        ...
        >
          {geometry.trunk ? (
            <path className="agent-route-trunk" d={geometry.trunk} />
          ) : null}
          {geometry.routes.map((route) => (
            <path
              ...
              className={
                active === route.id
                  ? "agent-route agent-route--active"
                  : "agent-route"
              }
            />
          ))}
        ...
        {copy.agents.map((item) => {
          ...
          return (
            <button
              ...
              className={
                active === item.id ? "agent-node is-active" : "agent-node"
              }
              style={{ left, top }}
