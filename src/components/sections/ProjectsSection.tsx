...
export function ProjectsSection({ copy }: { copy: RcsCopy["projects"] }) {
  return (
    <section
      className="technical-panel projects-section"
      id="projects"
      data-section="07"
    >
      <SectionHeading
      ...
      <div className="future-projects">
        <div className="future-projects__heading">
          <p className="eyebrow">{copy.futureEyebrow}</p>
          <h3>{copy.futureTitle}</h3>
          <p>{copy.futureLead}</p>
        </div>
        <div className="project-table">
          {copy.futureItems.map((project) => (
            <article
              className="project-row project-row--future"
              key={project.code}
              data-reveal
            >
              <div className="project-status">
